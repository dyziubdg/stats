...
STATS_SYSTEM_CONFIG = {
	...
	ignoredIds = {2554}, -- items with these ids will be banned from upgrading
	...
	rare_text = "RARE",
	rare_effect = true,
	rare_effect_id = CONST_ME_TUTORIALARROW,
	rare_loot_level = false, -- set to false if you want to disable levels on looted weapons
	...
	UPGRADE_SPELLS = {
		necklace = {
		    ...
		    {'regMP', 2, 3},
		    {'regHP', 2, 3},	
			 -- normalhp, normalmp, percenthp, percentmp
			 -- normalhp, normalmp, percenthp, percentmp			
		},
		helmet = {
	...
	STATS = {
		...
		
		{
			...
			enabledPercent = false -- hit% + hit%*x%
		},
	...
	if c ~= nil then
		if c:isContainer() then
			...
			if rares > 0 then
				...
				function rare_effect()
				Game.sendAnimatedText("RARE", pos, 198)
				doSendMagicEffect(pos, CONST_ME_TUTORIALARROW)
				end
				
					pos:sendMagicEffect(STATS_SYSTEM_CONFIG.rare_effect_id)
					addEvent(rare_effect, 2100, cid)
					addEvent(rare_effect, 4100, cid)
				end
			end
